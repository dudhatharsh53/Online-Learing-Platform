    ...
    return (
        <div className="container-lg py-10 animate-fade-in">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-slate-100 mb-1">Faculty Management</h1>
                    <p className="text-slate-500">{faculties.length} total faculty members</p>
                </div>
                    ...
                    {faculties.map((f) => (
                        <div key={f._id} className="card p-6 flex flex-col group hover:border-[#00a884]/30 transition-all duration-300">
                            <div className="flex items-start gap-4 mb-4">
                                <div className="w-16 h-16 rounded-2xl bg-slate-800 flex-shrink-0 flex items-center justify-center overflow-hidden border border-slate-700">
                                    {f.profileImage ? (
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-slate-100 truncate">{f.name}</h3>
                                    <p className="text-xs text-[#00a884] font-medium truncate">{f.email}</p>
                                </div>
                            </div>
                            <p className="text-slate-500 text-sm line-clamp-3 mb-6 flex-1 italic leading-relaxed">
                                "{f.description}"
                            </p>
                            <div className="flex items-center gap-2 pt-4 border-t border-slate-800/50">
                                ...
                                >
                                    {deleting === f._id ? <span className="w-4 h-4 border-2 border-red-400 border-t-transparent rounded-full animate-spin inline-block" /> : <FiTrash2 size={14} />}
                                </button>
                            </div>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="card w-full max-w-lg">
                        <div className="flex items-center justify-between p-6 border-b border-slate-800">
                            <h2 className="text-lg font-semibold text-slate-100">{modal === 'create' ? 'Add New Faculty' : 'Edit Faculty'}</h2>
